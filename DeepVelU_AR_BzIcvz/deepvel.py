...
class deepvel(object):
	...
	def __init__(self, observations1, observations2, observations3, output, border_x1=0, border_x2=0, border_y1=0, border_y2=0, same_as_training=0, network_path='network'):
		"""
		---------
		Keywords:
		---------
		...
		self.observations3 = observations3
	...
	def validation_generator(self):
		...
		if(self.same_as_training == 0):
			self.bz1_min = np.amin(self.observations1[:, self.border_x1:self.border_x1+self.nx, self.border_y1:self.border_y1+self.ny])
			self.bz1_max = np.amax(self.observations1[:, self.border_x1:self.border_x1+self.nx, self.border_y1:self.border_y1+self.ny])
			self.ic1_median = np.median(self.observations1[:, self.border_x1:self.border_x1+self.nx, self.border_y1:self.border_y1+self.ny])
			self.vz1_min = np.amin(self.observations3[:, self.border_x1:self.border_x1+self.nx, self.border_y1:self.border_y1+self.ny])
			self.vz1_max = np.amax(self.observations3[:, self.border_x1:self.border_x1+self.nx, self.border_y1:self.border_y1+self.ny])
		...
		while 1:
			for i in range(self.n_frames):
				...
				input_validation[:, :, :, 2] = (self.observations2[i * self.batch_size:(i + 1) * self.batch_size,
												self.border_x1:self.border_x1 + self.nx,
												self.border_y1:self.border_y1 + self.ny] - self.bz1_min) / self.ic1_median
				input_validation[:, :, :, 3] = (self.observations2[
												i * self.batch_size + 1:(i + 1) * self.batch_size + 1,
												self.border_x1:self.border_x1 + self.nx,
												self.border_y1:self.border_y1 + self.ny] - self.bz1_min) / self.ic1_median
				
				input_validation[:, :, :, 4] = (self.observations3[i * self.batch_size:(i + 1) * self.batch_size,
												self.border_x1:self.border_x1 + self.nx,
												self.border_y1:self.border_y1 + self.ny] - self.vz1_min) / (self.vz1_max - self.vz1_min)
				input_validation[:, :, :, 5] = (self.observations3[i * self.batch_size + 1:(i + 1) * self.batch_size + 1,
												self.border_x1:self.border_x1 + self.nx,
												self.border_y1:self.border_y1 + self.ny] - self.vz1_min) / (self.vz1_max - self.vz1_min)
...
if (__name__ == '__main__'):
	...
	parser.add_argument('-i3', '--in3', help='Input file')
	...
	imgs2 = f[0].data
	f = fits.open(parsed['in3'])
	imgs3 = f[0].data
	
	out = deepvel(imgs1, imgs2, imgs3, parsed['out'],
