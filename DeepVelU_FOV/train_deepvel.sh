python train_deepvel_unet_k2.py -a start -e 10 -o network/deepvel &
PID=$! #catch the last PID, here from command1
wait $PID #wait for command1, in background, to end
cp -r network network_e10
PID=$! #catch the last PID, here from command1
wait $PID #wait for command1, in background, to end
python train_deepvel_unet_k2.py -a continue -e 15 -o network/deepvel &
PID=$! #catch the last PID, here from command1
wait $PID #wait for command1, in background, to end
cp -r network network_e25
PID=$! #catch the last PID, here from command1
wait $PID #wait for command1, in background, to end
python train_deepvel_unet_k2.py -a continue -e 5 -o network/deepvel &
PID=$! #catch the last PID, here from command1
wait $PID #wait for command1, in background, to end
